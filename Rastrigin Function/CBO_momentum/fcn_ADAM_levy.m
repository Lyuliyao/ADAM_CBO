%% parameters
function vW_success_rate = fcn_ADAM_levy(dim,p_N,p_batch,simu_N,decay_rate,lambda,t_N)
%p_N = 10000
%p_batch = 100
...
%% main part
%while max(max(abs(diff)))>10^(-2)
while ((max(max(abs(diff)))>10^(-2))&&(t<50000))
    t = t+1;
    p_rp = randperm(p_N);
    f_temp = fcn_loss(W,B);
    ...
    m_bias = m/(1-beta1^t);
    v_bias = v/(1-beta2^t);
    Gradient = - lambda * m_bias./(sqrt(v_bias) + 1e-8)+   decay^(t/decay_rate)*R;
    R = R + scale*randn(size(W),'gpuArray');
    W = W +Gradient;
