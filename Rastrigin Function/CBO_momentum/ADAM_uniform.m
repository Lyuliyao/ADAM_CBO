%% parameters
function vW_success_rate = ADAM_uniform(dim,p_N,p_batch,simu_N,decay_rate,lambda)
%p_N = 10000
%p_batch = 100
...
while ((max(max(abs(diff)))>10^(-2))&&(t<50000))
    ...
    m_bias = m/(1-beta1^t);
    v_bias = v/(1-beta2^t);
    Gradient = - lambda * m_bias./(sqrt(v_bias) + 1e-8)+   decay^(t/decay_rate)*(rand(size(m_bias),'gpuArray')-0.5);
    W = W +Gradient;
